 ...
 * anterior.
 *
 * @author Pabloclavounclavito
 */
public class servicioPersona {
    //int dia, mes anio
    //date fecha = new Date(anio-1900,mes-1,dia);
    //v.setfechaNacimiento(fecha);
    ...
    public Persona crearPersona(){
        ...
        cp.setFechaNacimiento(fecha);
        return cp;
    }
    public int calcularEdad(Persona cp){
        Date hoy = new Date();
        return hoy.getYear()-cp.getFechaNacimiento().getYear();
    }
    public boolean menorQue(Persona cp){
        System.out.println("Ingresar otra edad");
        int menor = leer.nextInt();
        return (calcularEdad(cp) < menor);
    }
