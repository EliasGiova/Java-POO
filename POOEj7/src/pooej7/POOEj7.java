...
public class POOEj7 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < vectorPersona.length; i++){
            vectorPersona[i]= sp.CrearPersona();
            switch(sp.calcularIMC(vectorPersona[i])){
                case -1: bajopeso++;
                break;
                case 0: pesoideal++;
                break;
            ...
            if(sp.esMayorEdad(vectorPersona[i])){
                mayoredad++;
            }else{
                menoredad++;
            }
        }
        System.out.println("Personas con bajo peso " + (bajopeso*100)/vectorPersona.length + "%");
        System.out.println("Personas con peso ideal " + (pesoideal*100)/vectorPersona.length + "%");
        System.out.println("Personas con sobre peso " + (sobrepeso*100)/vectorPersona.length + "%");
        System.out.println("Personas mayores de edad " + (mayoredad*100)/vectorPersona.length + "%");
        System.out.println("Personas menores de edad " + (menoredad*100)/vectorPersona.length + "%");
    }
}
